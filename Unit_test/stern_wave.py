...
if __name__ == "__main__":
    ...
    for file_id in test_files:
        ...
        if gt_polys:
            ...
            for gt_poly in gt_polys:
                ...
                for i, pred in enumerate(pred_boxes):
                    ...
                    if iou > best_iou_for_this_boat:
                        ...
                        best_pred_idx = i
                if best_iou_for_this_boat >= IOU_THRESH:
                    tp += 1
            ...
            avg_iou = total_iou / tp if tp > 0 else 0.0

            print(f"Average IoU for {file_id}: {avg_iou:.4f}")
            print(f"TP={tp}, FP={fp}, FN={fn}")
            print(f"Precision={precision:.3f}, Recall={recall:.3f}, F1={f1:.3f}")

        if len(pred_boxes) == 0 or tp == 0:
            results.append({
                "image": file_id,
                "avg_iou": "X",
    ...
    plt.title("IoU / Execution Time Summary", pad=20)

    plt.savefig("iou_time_table_tradition.png", dpi=300, bbox_inches="tight")
